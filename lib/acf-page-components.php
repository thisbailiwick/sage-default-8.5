...
HTML;
	...
	function html_page_header_anchor_links($content, $id_tag) {
		$links = '';
		foreach ($content['links'] as $link) {
			$link = $link['link'];
			$links .= <<<HTML
				<a href="{$link['url']}" title="Link to the {$link['title']} section within this page">{$link['title']}</a>
HTML;
...
HTML;
	...
	function html_recordings($content, $id_tag) {
		$recordings = '';
		foreach ($content['recording'] as $recording) {
			$img = get_image_html($recording['image']);
			$links = [];
			foreach ($recording['purchase_links'] as $link) {
				$link = $link['link'];
				$links[] = <<<HTML
				<a href="{$link['url']}" title="Purchase at {$link['title']}">{$link['title']}</a>
HTML;
			}
